...
///
///         This contract does not hold user funds beyond mint payments,
///         which the owner can withdraw at any time via withdraw().
contract RunePass is ERC721, Ownable {
    ...
    /// @dev Free for the first FREE_SUPPLY mints, then costs MINT_PRICE.
    ...
    /// @dev Image is a simple on-chain SVG so the NFT never depends on
    ///      external storage like IPFS.
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory svgImage = _buildImage(tokenId, tier);
        string memory json = _buildJson(tokenId, tier, svgImage);

        return json;
    }

    function _buildImage(uint256 tokenId, string memory tier) internal pure returns (string memory) {
        string memory idText = tokenId.toString();
    ...
    function _buildJson(uint256 tokenId, string memory tier, string memory svgImage) internal pure returns (string memory) {
        bytes memory json = abi.encodePacked(
            "data:application/json,",
            "%7B%22name%22%3A%22RUNE%20Pass%20%23", tokenId.toString(), "%22%2C",
            "%22description%22%3A%22Soulbound%20membership%20pass%20for%20the%20RUNE%20intelligence%20terminal.%20Non-transferable.%22%2C",
            "%22attributes%22%3A%5B%7B%22trait_type%22%3A%22Tier%22%2C%22value%22%3A%22", tier, "%22%7D%5D%2C",
            "%22image%22%3A%22data%3Aimage%2Fsvg%2Bxml%2C", svgImage, "%22%7D"
        );
        return string(json);
    }
